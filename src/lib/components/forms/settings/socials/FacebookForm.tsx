...
export default function FacebookForm({ link }: { link?: string }) {
  const { toast } = useToast();
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const facebookLink = (e.currentTarget.firstChild as HTMLInputElement).value;
    try {
      const link = UrlSchema.parse(facebookLink);
      const response = await patchClientFetch(
        { social: "facebook", link: facebookLink },
        "/user/socials"
      );
      if (response.message) {
        toast({
          title: `Your Facebook link has been ${link ? "updated." : "added."}`,
          duration: 2000,
          variant: "success"
        });
      } else {
